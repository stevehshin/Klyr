...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const data: { displayName?: string; avatarData?: string | null; bio?: string | null; funFacts?: string | null } = {};
    if (typeof body.displayName === "string") data.displayName = body.displayName.trim().slice(0, 200) || null;
    if (body.avatarData !== undefined) data.avatarData = typeof body.avatarData === "string" ? body.avatarData.slice(0, 500000) : null; // cap data URL size
    ...
    const user = await prisma.user.update({
      where: { id: session.userId },
      data,
      select: { id: true, email: true, displayName: true, avatarData: true, bio: true, funFacts: true },
    });
