...
import { fetchRoomPresence, type PresenceParticipant } from "@/lib/call/presence";

const ROOM_JOINED_KEY = "klyr-loop-joined";

export interface TheRoomTileProps {
  tileId: string;
  roomLabel: string;
  userEmail?: string;
  onClose: () => void;
...
export function TheRoomTile({ tileId, roomLabel, userEmail, onClose }: TheRoomTileProps) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const list = await fetchRoomPresence(tileId);
  ...
  const handleLeave = useCallback(() => {
    ...
    try {
      ...
      localStorage.setItem(ROOM_JOINED_KEY, JSON.stringify(ids.filter((id) => id !== tileId)));
    } catch {}
  ...
  const handleJoin = () => {
    ...
    try {
      ...
      if (!ids.includes(tileId)) {
        localStorage.setItem(ROOM_JOINED_KEY, JSON.stringify([...ids, tileId]));
      }
    } catch {}
  ...
  const names = participants.map((p) => p.displayName || "Someone").join(", ");
    ...
    <div className="h-full flex flex-col rounded-lg overflow-hidden bg-transparent">
      <div className="tile-header flex items-center justify-between px-4 py-3 border-b border-gray-200/60 dark:border-gray-700/80 cursor-move bg-gray-50/80 dark:bg-gray-900/80 rounded-t-lg">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500 dark:text-gray-400">The Room</span>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center justify-center gap-4">
        {inRoom && (
          <div className="w-full text-sm text-gray-600 dark:text-gray-400 bg-gray-100/80 dark:bg-gray-800/80 rounded-lg px-3 py-2">
            <span className="font-medium text-gray-700 dark:text-gray-300">In the room:</span>{" "}
            {names || "—"}
          </div>
        )}
        {!joined ? (
          <>
            <p className="text-sm text-gray-600 dark:text-gray-400 text-center">
              Drop in anytime. Voice by default — turn on your camera in the call if you want.
            </p>
            <button
              onClick={handleJoin}
          ...
          <>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Voice in the call window. Turn on camera there if you want.
            </p>
          </>
        )}
      </div>
